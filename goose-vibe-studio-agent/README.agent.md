...
A self-hosted AI agent platform built on [goose](https://github.com/aaif-goose/goose), with a Streamlit chat UI, MinIO-backed app preview, and a dynamic deploy service (builder-kit-2).
...
| Streamlit UI | `Dockerfile.streamlit` | `8501` | Chat interface |
| builder-kit-2 | `services/builder-kit-2/Dockerfile` | `8080` | Preview/deploy service (reads from MinIO) |
...
docker run -d \
  --name goosed \
  -p 3005:3005 \
  -e GOOSE_SERVER__SECRET_KEY=sk-1234 \
  goosed:latest
```
...
docker run -d \
  --name goose-ui \
  -p 8501:8501 \
  -e GOOSE_URL=http://goosed:3005 \
  -e GOOSE_SERVER__SECRET_KEY=sk-1234 \
  goose-ui:latest
```

### 3. Run builder-kit-2 (preview service)

```bash
cd services/builder-kit-2
docker build -t builder-kit-2:latest .
...
| `GOOSE_SERVER__SECRET_KEY` | `sk-1234` | Must match goosed secret key |
| `GOOSE_WORKING_DIR` | `$PWD` | Working directory for agent sessions |
| `LITELLM_HOST` | `http://litellm.aipns.svc.cluster.local:4000` | LiteLLM proxy URL (pre-filled in UI) |
| `LITELLM_API_KEY` | `sk-1234` | LiteLLM master key (pre-filled in UI) |
...
3. goosed calls builder-kit-2's `POST /deploy` with the session ID
4. builder-kit-2 pulls all files from MinIO and serves them at `http://builder-kit-2:8080/apps/<session_id>/`
...
## Project Structure (this deployment)

```
goose/
├── Dockerfile.goosed          # goosed server image
├── Dockerfile.streamlit       # Streamlit UI image
├── streamlit_app.py           # Chat UI source
├── goosed-bin                 # Pre-built goosed binary
├── services/
│   └── builder-kit-2/         # Preview deploy service
│       ├── Dockerfile
│       ├── src/
│       │   ├── index.ts       # Express server, /deploy endpoint
│       │   ├── minio.ts       # MinIO S3 client
│       │   └── detector.ts    # Stack/framework detector
│       └── package.json
└── crates/
    └── goose/
        └── src/agents/platform_extensions/developer/
            └── minio.rs       # Minio upload integration (Rust)
```
